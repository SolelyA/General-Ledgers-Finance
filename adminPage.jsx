...
const AdminPage = () => {
    ...
    const [currentDate, setCurrentDate] = useState(new Date()) //Retrieved from: https://www.youtube.com/watch?v=sqb8A55wwJs
    const [expPasswords, setExpPasswords] = useState([])
    ...
    /*const fetchExpPasswords = async () => { based on code provided by Aaron Hannah
        ...
        try{
            ...
            setExpPasswords(users.map((user, index)=>{
                var CompareDate = new Date(user.passwordExp)
                alert("I am searching for passwords")
                console.log("Current date", currentDate)
                console.log("Expiration date ", CompareDate)
                if(currentDate.getUTCDate() > CompareDate.getUTCDate()){
                    console.log(user[index].userName)
                    alert("Found an expired password")
                    passwords.push(user)
                }
            }))

            if(passwords === null){
                alert("WAAAAAHHH! I DON WANNA PUT IN THESE PASSWORDS!!!")
            }
        ...
        <div>
            ...
            <h2>Expired Passwords</h2> 
                        ...
                        <Popup trigger={buttonPopup} setTrigger={setButtonPopup}>
                            {expPasswords && (
                                /*Based on table code from Ledger provided by Aaron Hannan*//*
                                <div>
                                    <h2>Expired Passwords</h2>
                                    <p> The following accounts have expired passwords. Please alert them to change their password as soon as possible</p>
                                    <table>
                                    <thead>
                                            <tr>
                                                <th>Username</th>
                                        ...
                                        <tbody>
                                            {expPasswords.map((user, index) => (
                                                <tr key={index}>
                                                    <td>{user.userName}</td>
                                                    <td>{user.email}</td>
                                                    <td>{user.passwordExp}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
